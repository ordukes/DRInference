...
run.sim <- function(N,nsim,seed,sigma){
  ...
  for(i in 1:nsim){#i=1
    ...
    x1<-1/(1+(exp(-20*(l[,1]-0.5))))
    a=rbinom(N,1,plogis(-1.5+cbind(x1,l4)%*%gamma))
    y=rnorm(N,cbind(x1,l4)%*%beta,1)
    ...
    #Lasso - misspecified
    G_lm<-rep(NA,N)
    l_inter<-cbind(model.matrix(object=y ~ .^4, data = data.frame(l))[,-1])
    lambda_init_G=lambda_select_binom(y=a,cov=l_inter)
    for (vfold in 1:nsplits){
      train <- s!=vfold; test <- s==vfold
      if (nsplits==1){ train <- test }
      modela_lm=glmnet(l_inter[train,],a[train],alpha=1,lambda=lambda_init_G,family="binomial")
      G_lm[test]=predict(modela_lm,l_inter[test,],type="response")
    } 
    M_lm<-rep(NA,N)
    lambda_init_M=lambda_select(y=y,cov=l_inter)
    for (vfold in 1:nsplits){
      train <- s!=vfold; test <- s==vfold
      if (nsplits==1){ train <- test }
      modely_lm=glmnet(l_inter[train,],y[train],alpha=1,lambda=lambda_init_M)
      M_lm[test]=predict(modely_lm,l_inter[test,],type="response")
    } 
    ...
    #Oracle estimator (used as a benchmark, to check super-efficiency)
    G_or<-plogis(-1.5+cbind(x1,l4)%*%gamma)
    M_or<-cbind(x1,l4)%*%beta
...
result250<-run.sim(N=250,nsim=1000,seed=569)
save.image("results_exp2")
result500<-run.sim(N=500,nsim=1000,seed=5)
save.image("results_exp2")
result1000<-run.sim(N=1000,nsim=1000,seed=897)
save.image("results_exp2")
result2000<-run.sim(N=2000,nsim=1000,seed=48)
save.image("results_exp2")
result3000<-run.sim(N=3000,nsim=1000,seed=106)
save.image("results_exp2")
result5000<-run.sim(N=5000,nsim=1000,seed=883)
save.image("results_exp2")
...
lines(ssize,rep(0,6),lty=2,lwd=0.7,col="darkgray")
axis(side=1,at=ssize,labels=T)
plot(ssize,size_mat[1:6,i+1],ylim=c(0,0.15),
